...
public class LightBomber : MonoBehaviour
{
    private void Awake()
    {
    ...
    public GameObject[,] AIPassiveSkill(GameObject[,] map)
    {
        ...
        if (HittedPiece==Piece)
        {
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 10; j++)
                {
                    if (FindShip(GameObject.Find("AIManager").GetComponent<AIShipPlacing>().AIMoneyMaker, GameObject.Find("AIManager").GetComponent<AIShipPlacing>().AITank,
    ...
    public GameObject[,] PassiveSkill(GameObject[,] map)
    {
        ...
        if (HittedPiece == Piece)
        {
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 10; j++)
                {
                    if (FindShip(GameObject.Find("MoneyMaker").GetComponent<MoneyMaker>(), GameObject.Find("Tank").GetComponent<Tank>(),
                                GameObject.Find("SideStep").GetComponent<SideStep>(), GameObject.Find("Faker").GetComponent<Faker>(),
                                GameObject.Find("Healer").GetComponent<Healer>(), GameObject.Find("LightBomber").GetComponent<LightBomber>(),
                                GameObject.Find("BombCatcher").GetComponent<BombCatcher>(), GameObject.Find("Bomber").GetComponent<Bomber>(),
                                GameObject.Find("Boomer").GetComponent<Boomer>(), GameObject.Find("FlameThrower").GetComponent<FlameThrower>(), i, j))
                    {
                        if (tank == true && map[i, j].GetComponent<Image>().color != Color.yellow && map[i, j].GetComponent<Image>().color != redColor)
                        {
                            map[i, j].GetComponent<Image>().color = Color.yellow;
    ...
    bool FindShip(MoneyMaker MoneyMaker, Tank Tank, SideStep SideStep, Faker Faker, Healer Healer, LightBomber LightBomber,
